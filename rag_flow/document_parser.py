...
class PDFParser:
    ...
    def parse(self, chunk_flag=False):
        ...
        if not self.images:
            ...
            df.to_csv("./output/extracted_df.csv", index=False, escapechar='\\')
            processor = PostProcessor(df)
            if chunk_flag:
                processor.main(chunk_flag=chunk_flag).to_csv("./output/extracted_df_chunks.csv", index=False, escapechar='\\')
            else:
                processor.main(chunk_flag=chunk_flag).to_csv("./output/extracted_df_processed.csv", index=False, escapechar='\\')
            return True
        else:
...
if __name__ == "__main__":
    fp = "/content/Quanto.pdf"
    # fp = "/content/New_WagonR_CNG_Supplemnetary_owners_manual _99011M69R70-74E.pdf"
    parser = PDFParser(fp)
